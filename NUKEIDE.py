...
import subprocess
allextensions = "THEMES"
# create an instance for window
window = Tk()
# set title for window
window.title("Nuke IDE")
...
editor = ScrolledText(window, font=("ariel 13"), wrap=None)
...
exten_menu = Menu(menu, tearoff=0)
# add menu labels
...
menu.add_cascade(label="Manage Extensions",menu=exten_menu)
...
run_menu.add_command(label="Run", accelerator="Ctrl+R", command=run)
# add commands in extension menu
def check_extentions():
    allextensions
    showinfo("Extensions",allextensions)
exten_menu.add_command(label="see extentions",command=check_extentions)
...
status_bars = ttk.Label(window,text = "https://github.com/superpythonguy/Nuke-IDE \t\t\tV0.1.0\t\t\t characters: 0 words: 0")
status_bars.pack(side = BOTTOM)
...
def change_word(event = None):
    ...
    if editor.edit_modified():
        ...
        status_bars.config(text = f"https://github.com/superpythonguy/Nuke-IDE \t\t\tV0.1.0\t\t\t characters: {chararcter} words: {word}")
...
def light_blue():
    ...
    output_window.config(fg="white", bg="#3F425D")
def off_white():
    editor.config(fg="black",bg="#D6E5E4")
    output_window.config(fg="black", bg="#D6E5E4")
#import extension
from themes import devdark,monkai
# add commands to change themes

theme_menu.add_command(label="Light", command=light)
theme_menu.add_command(label="midnight Black", command=dark)
theme_menu.add_command(label="Light Blue",command=light_blue)
theme_menu.add_command(label="Off white",command=off_white)
theme_menu.add_command(label="devdark",command=devdark)
theme_menu.add_command(label="monkai",command=monkai)
# create output window to display output of written code
output_window = ScrolledText(window, height=12)
